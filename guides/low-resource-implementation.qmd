...
Each card set follows the same pedagogical pattern: AI demonstrates capability, AI acknowledges limitation, human judgment required. These materials are available in the [printables directory](../printables/index.qmd#activity-1-security-detective-teams).
...
Each card set aligns with the grade-specific incident scenarios (Fix It Team, Computer Problem Solvers, School Incidents, Enterprise SOC). These materials are available in the [printables directory](../printables/index.qmd#activity-3-ai-assisted-incident-response).
...
Each card set presents the AI's perspective on policy decisions appropriate to the grade-band scenario (Robot Helper Rules, Computer Rules Committee, SchoolGuard Governance, Enterprise Security Policy). These materials are available in the [printables directory](../printables/index.qmd#activity-2-ethics-in-automated-security).
